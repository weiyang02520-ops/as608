...
//说明:  模块返回确认字
// 写记事本 (修复了 temp 未初始化的 Bug)
uint8_t GZ_WriteNotepad(uint8_t NotePageNum, uint8_t *Byte32)
{
    uint16_t temp = 0;  // <--- 就是这里！标准库里修复了，必须初始化为0
    uint8_t ensure, i;
    uint8_t *data;
    ...
    SendAddr();                      // 发送模块地址
    SendFlag(0x01);                  // 命令包标识
    SendLength(36);                  // 包长度 = 指令(1) + 页号(1) + 32字节数据 + 校验和(2) = 36
    Sendcmd(0x18);                   // 写记事本指令
    Com_SendData(NotePageNum);       // 发送页号

    // 计算校验和：包标识(0x01) + 包长度(36) + 指令码(0x18) + 页号 + 所有数据字节
    ...
    SendCheck(temp);                 // 发送校验和（高8位，低8位）

    data = JudgeStr(2000);           // 等待应答
    if (data)
        ensure = data[9];
    else
        ensure = 0xff;
